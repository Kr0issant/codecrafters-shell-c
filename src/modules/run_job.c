...
int run_cmd(Command cmd, char **cwd, char *output_file, int fd, int append, int is_pipe, int is_bg) {
    ...
    if (exe_path != NULL) {
        Program exit = run_program(exe_path, cmd.tokens, output_file, fd, append, is_bg);
        if (is_bg) printf("[1] %d\n", exit.pid);
        free(exe_path);
        return exit.status;
    }
